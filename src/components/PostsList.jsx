...
export default function PostsList({ posts }) {
  ...
  const { setElements } = useObserver({ rootMargin: "0px", match: "/posts", onlyOnShow: true, handleVisibility })

  useEffect(() => {
    const elements = document.querySelectorAll('article[name="postLink"]:not([data-visible="true"])')
    setElements(elements)
  }, [postsToShow])

  const postsElements = posts.slice(0, postsToShow).map( ( post, index) => {
    return  <PostCard key={post.id} link={true} post={post} last={index === postsToShow - 2} />
  })
